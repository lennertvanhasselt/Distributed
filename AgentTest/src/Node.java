...
public class Node extends UnicastRemoteObject implements NodeInterface {
	...
	public void initializeAgentFileList() throws MalformedURLException, RemoteException, NotBoundException{
	...
	public void agentUpdateFileList() throws UnknownHostException{
		ArrayList<FileInfo> TotalFileList = new ArrayList<FileInfo>();
		
		Iterator<FileInfo> it1 = deletedFiles.iterator();
		
		while(it1.hasNext()){
			if(TotalFileList.contains(it1.next())){
				TotalFileList.remove(it1.next());
	            deletedFiles.remove(it1.next());
			}
		}
		
		Iterator<FileInfo> it2 = replicatedFiles.iterator();
